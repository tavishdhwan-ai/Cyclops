  ...
  const getEventIcon = (type: ActivityEvent['eventType']) => {
    switch (type) {
      ...
      case 'document_analysis':
        return <FileSearch className="h-4 w-4 text-zinc-400" />;
      case 'auth_event':
        return <Key className="h-4 w-4 text-zinc-400" />;
      case 'policy_change':
        return <Sliders className="h-4 w-4 text-zinc-400" />;
      default:
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1 rounded-lg border border-zinc-800 bg-zinc-950 p-1">
            <span className="text-[11px] text-zinc-400 ml-1.5 mr-1">Category:</span>
            {[
              { id: 'all', label: 'All' },
          ...
          <div className="flex items-center gap-1 rounded-lg border border-zinc-800 bg-zinc-950 p-1">
            <span className="text-[11px] text-zinc-400 ml-1.5 mr-1">Severity:</span>
          ...
          {filteredActivities.length > 0 ? (
            filteredActivities.map((act) => (
              <div
                ...
                className="flex items-center justify-between p-4 hover-card cursor-pointer hover:bg-zinc-800/40 transition-colors gap-4"
              >
                <div className="flex items-center gap-3.5 min-w-0">
                  <div className="h-9 w-9 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-xs font-bold text-zinc-300 shrink-0">
                    {act.employeeName.charAt(0)}
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-xs text-zinc-200">{act.employeeName}</span>
                      <span className="text-zinc-600">•</span>
                      <span className="text-xs text-zinc-400 font-medium">{act.title}</span>
                    </div>
          ...
          <div className="space-y-4">
            <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-4 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-zinc-400">Event Title</span>
                <Badge level={selectedActivity.severity}>{selectedActivity.severity}</Badge>
              </div>
              <p className="text-sm font-bold text-zinc-100">{selectedActivity.title}</p>
              <p className="text-xs text-zinc-300">{selectedActivity.description}</p>
            </div>

            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3">
                <span className="text-[10px] text-zinc-500 block">Employee & Role</span>
                <p className="font-semibold text-zinc-200 mt-0.5">{selectedActivity.employeeName}</p>
                <p className="text-[10px] text-zinc-400">{selectedActivity.role}</p>
              </div>
              <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3">
                <span className="text-[10px] text-zinc-500 block">IP Address</span>
                <p className="font-semibold font-mono text-zinc-200 mt-0.5">{selectedActivity.ipAddress}</p>
              </div>
              <div className="col-span-2 rounded-lg border border-zinc-800 bg-zinc-950 p-3">
                ...
                <p className="font-semibold text-zinc-200 mt-0.5">{selectedActivity.device}</p>
              </div>
            </div>
            ...
            <div className="flex justify-end pt-2 border-t border-zinc-800">
              <button
                ...
                className="rounded-lg border border-zinc-800 bg-zinc-900 px-4 py-1.5 text-xs text-zinc-300 hover:bg-zinc-800"
              >
                Close Record
              </button>
            </div>
