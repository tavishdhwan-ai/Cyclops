    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <Card className="space-y-4">
        <div
          ...
          className="border-2 border-dashed border-zinc-800 hover:border-zinc-600 rounded-xl p-8 text-center cursor-pointer bg-zinc-950/60 hover:bg-zinc-900/40 transition-colors space-y-3"
        >
          <div className="h-12 w-12 rounded-full border border-zinc-700 bg-zinc-900 flex items-center justify-center mx-auto text-zinc-300">
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-zinc-800/60">
          <span className="text-[11px] text-zinc-400">Test Sample Files:</span>
          <button
        ...
        <Card className="text-center py-8 space-y-3">
          ...
          <div>
            <p className="text-xs font-semibold text-zinc-200">
              Deconstructing File: <span className="font-mono text-zinc-300">{selectedFile}</span>
            </p>
            <p className="text-[11px] text-zinc-400 mt-0.5">{scanStage}</p>
          </div>
        </Card>
      )}
        ...
        <Card className="space-y-6 border-zinc-700/60">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-4">
            <div className="flex items-start gap-3">
              <div className="h-10 w-10 rounded-lg bg-zinc-900 border border-zinc-700 flex items-center justify-center text-zinc-300 shrink-0 mt-1">
                <FileText className="h-5 w-5" />
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-base font-bold text-zinc-100 font-mono">{activeDoc.fileName}</h2>
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 space-y-1.5 text-xs">
            ...
            <p className="font-mono text-[11px] text-zinc-300 break-all">{activeDoc.fileHash}</p>
          </div>

          {/* AI Safety Summary */}
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-4 space-y-2">
            <div className="text-xs font-semibold text-zinc-200">SentinelAI Safety Verdict</div>
            <p className="text-xs text-zinc-300 leading-relaxed">{activeDoc.aiSummary}</p>
          </div>

          {/* Detected Signals */}
          <div className="space-y-2">
            <h3 className="text-xs font-semibold text-zinc-300">File Inspection Signals</h3>
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 flex items-center justify-between text-xs">
            <div>
              <span className="text-[10px] text-zinc-500 uppercase font-semibold">Recommended Action</span>
              <p className="font-semibold text-zinc-200 mt-0.5">{activeDoc.recommendedAction}</p>
            </div>
          </div>
        </Card>
      )}
          ...
          {docScans.map((doc) => (
            ...
            >
              <div className="min-w-0 pr-4">
                <p className="text-xs font-semibold text-zinc-200 truncate font-mono">{doc.fileName}</p>
